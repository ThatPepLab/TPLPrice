(() => {
  ...
  const productKey = (value) => String(value || "").toLowerCase()
    ...
    .replace(/ahk[\\s-]*cu/g, "ahkcu")
    .replace(/vasoactive\\s+intestinal\\s+peptide|\\bvip\\b/g, "vip")
    .replace(/lysine[\\s-]*proline[\\s-]*valine|\\bkpv\\b/g, "kpv")
    .replace(/fox[o0]4[\\s-]*dri/g, "foxo4dri")
    .replace(/slu[\\s-]*pp[\\s-]*332/g, "slupp332")
    .replace(/peg[\\s-]*mgf/g, "pegmgf")
    .replace(/mots[\\s-]*c(?:\\s*human)?/g, "motsc")
    .replace(/melanotan\\s*(?:ii|il|2)/g, "melanotan2")
    .replace(/tesa(?:morelin)?\\s*(?:10)?\\s*\\+?\\s*ipa(?:morelin)?\\s*(?:5)?/g, "tesamorelinipamorelin")
    .replace(/cagri(?:lintide)?\\s*(?:sema|semaglutide)/g, "cagrilintidesemaglutide")
    .replace(/epit(?:halon)?(?:\\s*20)?/g, "epithalon")
