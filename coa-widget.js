(() => {
  ...
  const markup = (product, strength, vendor) => {
    ...
    if (result.completed.length) {
      ...
      const status = statusInfo(result.completed[0]);
      ...
      return '<button type="button" class="coa-status coa-complete"' + attrs + ">" + esc(label) + '<span class="coa-result-status coa-result-' + status.key + '">' + esc(status.label) + "</span>" + (newerPending ? "<span>Newer test pending</span>" : "") + "</button>";
    }
